...
class Player:
    ...
    Incluye planificación de rutas usando RRT*, control de velocidad, manejo de pelota
    y navegación autónoma con evitación de obstáculos.

    Attributes:
        id                      (int):              Identificador único del jugador.
        x                       (int):              Posición actual en coordenada x.
        y                       (int):              Posición actual en coordenada y.
        angle                   (float):            Ángulo actual en grados (0-360).
        team                    (str):              Equipo al que pertenece ('red' o 'blue').
        distance2ball           (float):            Distancia al balón en píxeles.
        anglerobot2ball         (float):            Ángulo hacia el balón en grados.
        rol                     (int):              Rol del jugador (0: defensivo, 1: atacante).
        max_speed               (int):              Velocidad máxima de movimiento.
        max_rotation_speed      (int):              Velocidad máxima de rotación.
        current_path            (list):             Ruta actual planificada como lista de puntos.
        goal                    (numpy.ndarray):    Posición objetivo actual.
        ball_capture_distance   (int):              Distancia para capturar el balón.
    """

    def __init__(self, player_id, x, y, angle, team='red'):
        """Inicializa un nuevo jugador con parámetros básicos.
        ...
        Args:
            ...
            team (str, optional): Equipo del jugador. Defaults to 'red'.
        """
        ...
        self.planner = RrtStarSmart(step_len=20, goal_sample_rate=0.05, search_radius=50, iter_max=500)
    ...
    def move_to_position(self, target_position, speed_factor=1.0):
        ...
        Planifica una ruta si es necesario y ejecuta el movimiento hacia la posición
        objetivo usando el algoritmo RRT* o movimiento directo.

        Args:
            target_position (array-like): Posición objetivo [x, y].
            speed_factor (float, optional): Factor de velocidad (0.0 a 1.0). Por defecto 1.0.

        Note:
            Si no existe una ruta válida o el objetivo cambió significativamente,
            se ejecuta una nueva planificación de ruta.
        """
    ...
    def _move_directly_to(self, target_position):
        ...
        Realiza un movimiento en línea recta hacia la posición objetivo sin
        considerar obstáculos. Usado como respaldo cuando no hay ruta planificada.

        Args:
            target_position (array-like): Posición objetivo [x, y].

        Note:
            Este método se usa como respaldo cuando no hay ruta planificada
            o para movimientos simples de corta distancia.
    ...
    def _orient_towards(self, target_position):
        """Orienta al jugador hacia una posición específica.

        Args:
            target_position (array-like): Coordenadas [x, y] hacia las que orientar.

        Note:
            La rotación está limitada por max_rotation_speed para simular
            restricciones físicas realistas del robot.
        """
    ...
    def _follow_path(self):
        """Sigue el camino planificado actual moviéndose al siguiente waypoint.

        Note:
            Elimina waypoints alcanzados automáticamente. Si no hay ruta
            disponible, el método no ejecuta ninguna acción.
        """
    ...
    def has_ball(self):
        ...
        Returns:
            bool: True si el jugador controla la pelota, False en caso contrario.
        """
        return self._has_ball

    def intercept_ball(self, ball, prediction_time=0.5):
        """Intercepta la pelota en su trayectoria futura predicha.

        Args:
            ball (Ball): Instancia de la pelota a interceptar.
            prediction_time (float, optional): Tiempo de predicción en segundos.
                Por defecto 0.5.

        Note:
            Si la pelota no tiene información de velocidad, se mueve hacia
    ...
    def set_goal(self, goal_position):
        """Establece una nueva meta y planifica la ruta óptima.

        Args:
            goal_position (array-like): Coordenadas [x, y] del objetivo.

        Note:
            Activa la replanificación si el tiempo mínimo entre planificaciones
            ha transcurrido.
        """
        self.goal = goal_position
    ...
    def get_position(self):
        ...
        Returns:
            np.ndarray: Array con las coordenadas [x, y] actuales.
        """
    ...
    def angle_difference_ball(self, ball):
        ...
        Args:
            ball: Instancia de pelota con el que se calcula la diferencia angular
