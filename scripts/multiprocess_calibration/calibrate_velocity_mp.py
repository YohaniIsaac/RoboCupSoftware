"""Módulo de apoyo para calibración de velocidad con multiprocessing.

Este módulo proporciona la función run_calibration() que ejecuta dos procesos independientes:
1. Proceso de percepción: Detección continua de robots con ArUco
2. Proceso de control: Comandos RF, UI y control de teclado

La comunicación entre procesos es no bloqueante usando pipes.

Este módulo NO debe ejecutarse directamente. Usar scripts/calibrate_velocity_control.py
"""
...
from multiprocess_calibration.control_process import control_loop

log = logging.getLogger(__name__)


def run_calibration(robot_id, serial_port='/dev/ttyUSB0', camera_id=None):
    """Ejecuta la calibración con multiprocessing.
    ...
    try:
        log.info("=" * 70)
        log.info("CALIBRACIÓN DE VELOCIDAD - MULTIPROCESSING")
        log.info("=" * 70)
        ...
        # PROCESO 2: Control (UI, teclado, RF)
