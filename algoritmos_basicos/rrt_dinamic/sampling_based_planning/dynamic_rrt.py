...
class DynamicRrt:
    def __init__(self, s_start, s_goal, step_len, goal_sample_rate, waypoint_sample_rate, iter_max, lista_obs):
        ...
        self.env = env.Env(lista_obs)
        # self.plotting = plotting.Plotting(s_start, s_goal, lista_obs)
        self.utils = utils.Utils(lista_obs)
 ...
 # Realiza la planificación inicial de la ruta utilizando el algoritmo RRT
    def planning(self):
...
# responde a eventos del ratón. Permite agregar un obstáculo haciendo clic en la interfaz gráfica.
...
# Marca los nodos como inválidos si están en colisión con el nuevo obstáculo.
    def InvalidateNodes(self):
        for edge in self.edges:
            if self.is_collision_obs_add(edge.parent, edge.child):
                edge.child.flag = "INVALID"

# Comprueba si algún nodo en la ruta está marcado como inválido.
    def is_path_invalid(self):
        for node in self.waypoint:
            if node.flag == "INVALID":
                return True

# Verifica si una arista (definida entre dos nodos) colisiona con el nuevo obstáculo añadido.
...
# Replanifica la ruta si algún nodo de la ruta original ha sido invalidado.
    def replanning(self):
        self.TrimRRT()
...
# Elimina los nodos y aristas que están marcados como inválidos.
    def TrimRRT(self):
...
# Genera un nodo aleatorio para la expansión del árbol.
...
# Genera un nodo aleatorio en el contexto de la replanificación.
...
# Encuentra el nodo más cercano en la lista de nodos al nuevo nodo generado.
    @staticmethod
    def nearest_neighbor(node_list, n):
        return node_list[int(np.argmin([math.hypot(nd.x - n.x, nd.y - n.y)
                                        for nd in node_list]))]

# Expande el árbol al intentar conectar el nodo más cercano con el nuevo nodo generado.
...
# Extrae la ruta completa desde el nodo objetivo hacia atrás hasta el nodo inicial.
...
# Extrae una lista de puntos intermedios (waypoints) en la ruta.
...
def main():
    ...
    drrt = DynamicRrt(x_start, x_goal, 50, 0.1, 0.6, 5000, lista_obstaculos)
    drrt.planning()
