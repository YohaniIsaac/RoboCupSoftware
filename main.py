...
def busqueda_ball(conn):
    # Color          
    naranjo= ((10, 100, 20), (30, 255, 255))  # Rango de color para el naranjo

    first_frame = True
    while True:
        frame = conn.recv()  # Recibir datos como bytes a través de la tubería
...
if __name__ == '__main__':
    ...
    # Crear los procesos
    p1 = multiprocessing.Process(target=make, args=(conn1, conn3))
    p2 = multiprocessing.Process(target=busqueda_ball, args=(conn2,))
    p3 = multiprocessing.Process(target=busqueda_player, args=(conn4,))
