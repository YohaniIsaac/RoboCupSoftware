...
class BehaviorManager:
    ...
    """
    def __init__(self, players, ball, team='red', use_real_robots=False, serial_port='/dev/ttyUSB0'):
        """
        ...
        # Inicializar logger
        self.logger = logging.getLogger("ai.behavior_tree.manager")
    ...
    def update(self):
        ...
        for player in self.team_players:
            ...
            if player.rol == ROL_ATACANTE:
                ...
                self.logger.debug(f"Árbol atacante para jugador {player.id} completado con estado {status}")
            else:  # ROL_DEFENSIVO
                status = self.defender_tree.tick(blackboard)
                self.logger.debug(f"Árbol defensor para jugador {player.id} completado con estado {status}")
